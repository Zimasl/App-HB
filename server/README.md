...
native/
  ...
  delete_account.php          — удаление аккаунта через SMS-подтверждение
  account_deletion.html       — публичная страница с инструкцией по удалению аккаунта
  privacy_policy.json         — документ политики конфиденциальности для приложения
...
- `native/delete_account.php` — серверный endpoint для удаления аккаунта с подтверждением по SMS.
- `native/account_deletion.html` — публичная страница, которую можно использовать в App Store / Google Play как страницу удаления аккаунта.
- `native/privacy_policy.json` — версия политики конфиденциальности для показа внутри приложения, включая раздел про удаление аккаунта.

### Что опубликовать

1. Выложите `delete_account.php` в `https://hozyain-barin.ru/native/delete_account.php`.
2. Выложите `account_deletion.html` в `https://hozyain-barin.ru/native/account_deletion.html`.
3. При необходимости обновите `privacy_policy.json` по адресу `https://hozyain-barin.ru/native/privacy_policy.json`.

### Что проверить

- В профиле приложения открывается действие `Удалить аккаунт`.
- После запроса кода приходит SMS.
- После подтверждения кодом аккаунт удаляется, сессия завершается, локальные данные очищаются.
- Публичная страница удаления аккаунта доступна без авторизации.
...
1. **Скопировать на сервер**  
   ...
   - `delete_account.php` → `https://hozyain-barin.ru/native/delete_account.php`
   - `account_deletion.html` → `https://hozyain-barin.ru/native/account_deletion.html`
   - `privacy_policy.json` → `https://hozyain-barin.ru/native/privacy_policy.json`
